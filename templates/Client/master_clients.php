 ...
 <div class="container-xxl flex-grow-1 container-p-y">
     ...
     <div class="card">
         <div class="card-header align-items-center row">
                ...
                <div class="row mt-2">
                    <div class="col-lg-2 col-sm-12 mb-2">
                        <label for="from">Manager:</label>
                        <select name="assign_manager_id" class="form-select js-example-client">
                            <option value="0">Select Account Manager</option>
                            <!-- <option value="none_assigned" <?= ($assign_manager_id == 'none_assigned') ? 'selected' : '' ?>>None Assigned</option> -->
                            <?php foreach ($account_manager as $val) : ?>

                                <option value="<?= $val->id ?>" <?= ($val->id == $assign_manager_id) ? 'selected' : '' ?>>
                    ...
                    <div class="col-lg-2 col-sm-12 mb-2">
                        <label for="internalHold">Internal Hold:</label>
                        <select id="internalHold" name="internal_hold" class="form-select">
                            <option value="0">Select Hold Status</option>
                            <option value="on_hold" <?= $internal_hold == 'on_hold' ? 'selected':'' ?>>on hold</option>
                            <option value="active" <?= $internal_hold == 'active' ? 'selected':'' ?>>Active</option>
                        </select>                        
                    </div>
                    ...
                    <?php if (($user->user_type == 1 && $permission == 2) || $user->user_type == 0) { ?>
                        <div class="col-lg-2 col-sm-12 mb-2 text-end">
        ...
        <div class="m-2 checkAll row" style="display:none;">
            <div class="col-sm-12 mb-4">
                <?= $this->Form->create(null, ['method' => 'post', 'action' => 'Client/bulkUpdatemanager']) ?>
                    <input type="hidden" name="bulkClientId" id="bulkClientId" value="">   
                    <div class="row">
                            <div class="col-md-4">
                            <label class="form-label" for="basic-icon-default-company">Account Manager</label>
                            <select  name="bulk_assign_manager" id="assignBulkManager" class="form-select" required>
                                <option value="0">Select Account Manager</option>
                                <?php foreach ($account_manager as $val) : if($val->capacity > $val->store_assign) { ?>
                                <option value="<?= $val->id.'-'.($val->capacity - $val->store_assign) ?>"><?= ucfirst($val->first_name) ?>
                                    <?= ucfirst($val->last_name).'('.$val->capacity - $val->store_assign.')' ?></option>
                                <?php } endforeach ?>
                            </select>
                        </div>
                        <span class="error" id="caper"></span>
         ...
         <div class="container">
            <div class="table text-nowrap table-responsive">
                 <table id="masterClient" class="display table table-sm" style="width:100%">
                     <thead>
                         <tr>
                            <?php if (($user->user_type == 1 && $user->role_id == 5) || $user->user_type == 0) { ?>
                             <th class="no-export"><input type="checkbox" id="selectall" value="0" onclick="selectBulkClient()"></th> 
                            <?php } ?> 
                             <th>Name</th>
                             <th>Email</th>
                             <th>Phone</th>
                             ...
                             <!-- <th>Affiliate</th> -->
                            <th>Internal Hold</th>
                            <th>Internal Hold Date</th>
                             <th class="no-export">Booking</th>
                     ...
                     <tbody class="table-border-bottom-0">
                         <?php foreach ($masterClients as $val) : ?>
                             <tr>
                                <?php if (($user->user_type == 1 && $user->role_id == 5) || $user->user_type == 0) { ?>
                                <td>
                                    <input type="checkbox" class="feecheckbox" name="client_ids[]" value="<?= $val->id ?>" onclick="clientBulkManagerUpdate()">
                                </td>
                                <?php } ?>
                                 ...
                                 <!-- <td><?= ($val->organisation_name) ? ucfirst($val->organisation_name): '' ?></td> -->
                                 <td><?= $val->manager ?></td>
                                 <!-- <td><?= $val->affiliate_client == 1 ? 'Yes' : 'No' ?></td> -->
                                 <td>
                                    <?php $status = $val->internal_hold_date ? 1 : 0  ?> 
                                     <div class="form-check form-switch mb-2">
                                         <input class="form-check-input" type="checkbox" <?= $status == 1 ? 'checked' : 'unchecked' ?> onclick="updateappointment(<?= $val->id ?>,<?= $status ?>,'Users/updateInternalHold')"/>

                                     </div>
                                 </td>
                                 <td data-sort="<?= $val->internal_hold_date ? date('YmdHis', strtotime($val->internal_hold_date)) : '' ?>">
                                     <?= $val->internal_hold_date ? date('m/d/Y', strtotime($val->internal_hold_date)) : '---' ?>
                                 </td>
                                 <td>
                                     <div class="form-check form-switch mb-2">
                                         <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" <?= $val->is_appointment == 1 ? 'checked' : 'unchecked' ?> <?= ($user->user_type == 1 && $bookPermission == 3) ? 'onclick="return false"' : 'onclick="updateappointment(' . $val->id . ',' . $val->is_appointment . ',\'Users/updateappointment\')"'; ?> />
                                 ...
                                 <td>
                                     <div class="dropdown">
                                         ...
                                         <?php
                                            if ($user->user_type == 1 && $permission == 1) {
                                                $d = 'View';
                                            } else {
                                                $d = 'Edit';
                                            }
                                            ...
                                            if (($user->user_type == 1 && $lgPermission == 1) || $user->user_type == 0) { ?>
             ...
             <div class="modal-body">
                 ...
                 <!-- <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label" for="basic-icon-default-company">Account Manager</label>
                            <select name="assign_manager" class="form-select" style="width: 100%" required>
                                <option value="0">Select Account Manager</option>
                                <?php foreach ($account_manager as $val) : ?>
                                <option value="<?= $val->id ?>"><?= ucfirst($val->first_name) ?>
                                    <?= ucfirst($val->last_name) ?></option>
