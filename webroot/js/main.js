...
(function(undefined) {
    ...
    var scannerLaser = Q(".scanner-laser"),
        ...
        scanningResult = Q("#scanning-Result"),
        cationDiv = Q(".caption"),
        formDiv = Q(".formData"),
        bartext = Q("#bartext"),
        grabImg = Q("#grab-img"),
    ...
    var args = {
            ...
            if(res.code === '0')
            {
                ...
                if(y !== res.code)
                {
                    ...
                    /* Set the type attribute */
                        ...
                        x.setAttribute("name", "barcode[]");
                    /* Set the value to the attribute */
                    x.setAttribute("value", res.code);
                    formDiv.appendChild(x);
                    bartext.value = res.code;


                    var row = cationDiv.insertRow(1);
                    var cell1 = row.insertCell(0);
                    var cell2 = row.insertCell(1);
                    cell1.innerHTML = res.code;
                    cell2.innerHTML = `<i class='bx bx-trash me-1' onclick='removeRow(this,"`+res.code+`")'></i>`;
                
                }
                scanningResult[txt] = "Stopped";
                decoder.stop()
            }
        ...
        cameraError: function(error) {
            var p, message = "Error detected with the following parameters with camera:\n";
