...
var WebCodeCamJS = function(element) {
    ...
    var display = Q(element),
        ...
        options = {
            ...
            cameraSuccess: function(stream) {
                ...
                document.getElementById("webcodecam-canvas").width = 320; // Desired width
                document.getElementById("webcodecam-canvas").height = 320;
            },
    ...
    function setEventListeners() {
        ...
        video.addEventListener('canplay', function(e) {
            if (!isStreaming) {
                console.log('vedio width='+videoWidth)
                if (video.videoWidth > 0) {
                    h = video.videoHeight / (video.videoWidth / w);
                }
                display.setAttribute('width', 320);
                display.setAttribute('height', 320);
                isStreaming = true;
        ...
        video.addEventListener('play', function() {
            setInterval(function() {
                if (!video.paused && !video.ended) {
                    ...
                    var img = (display.toDataURL()).split(',')[1]
                    myDisplay(img)
                }
    ...
    async function myDisplay(input) {
        ...
        if (!video.paused && !video.ended) {
            ...
            let myPromise = new Promise(function(resolve, reject) {
                var jqXHR = $.ajax({
                    type: "POST",
                    url: "https://dev-portal.ecomauthority.com/bars",
                    data: { mydata: input },
                    async: true,
                    xhrFields: {
    ...
    async function buildSelectMenu(selectorVideo, ind) {
        videoSelect = Q(selectorVideo);
        videoSelect.innerHTML = '';
        try {
                await mediaDevices.getUserMedia({video: true}); 
            if (mediaDevices && mediaDevices.enumerateDevices) {
                await mediaDevices.enumerateDevices().then(function(devices) {
                    console.log('hello')
